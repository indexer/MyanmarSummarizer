...
def compare_sents_bounded(sent1, sent2):
    ...
    UPPER_BOUND, it returns 0 instead"""
...
def summarize_block(block):
    ...
    if not block:
        return None
    sents = block.split(mm_sentence_tokenizer)
    word_sents = list(mm_tokenizer.tokenize(sents))
...
def summarize_page(url):
    ...
    html = bs4.BeautifulSoup(requests.get(url).text,"html5lib")
